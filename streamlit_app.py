...
class TableState:
    ...
    ui_cmd: Optional[str] = None  # pending UI command (one-shot)
    ui_info: str = ""  # last info/validation message
    ui_prompt: str = ""  # current prompt text
    ...
    bankroll_base: Dict[str, int] = field(default_factory=dict)  # buy-ins/deposits baseline for session P/L
...
def input_with_commands(prompt: str, h: Optional[HandState], t: TableState, viewer: Optional[str]) -> str:
    """Streamlit input adapter.
    - Reads one command from t.ui_cmd.
    - Supports cheat codes at any prompt.
    - If no input yet, raises NeedHumanInput to pause the simulation.
    """
    while True:
        t.ui_prompt = prompt
        if not t.ui_cmd:
        ...
        if consumed:
            # If cheat code reveals, show folded for this render only
            if line.lower().strip() in ("reveal", "show out"):
                t.reveal_folded_once = True
            t.ui_info = msg
            # pause so UI can re-render and ask again
            raise NeedHumanInput(prompt)
...
def _init_table(n_ai: int, starting_stack: int, sb: int) -> TableState:
    hero = "Jack"
    ...
    t = TableState(
        ...
        reveal_once=False,
        reveal_folded_once=False,
        overlay_once=False,
    )
    # bankroll base for P/L
    t.bankroll_base = {nm: stacks[nm] for nm in stacks}
    return t


def _start_new_hand(t: TableState, dealer_idx: int) -> HandState:
    cleanup_busted(t)
    dealer_idx = dealer_idx % max(1, len(t.players))
    h = deal_new_hand(t, dealer_idx=dealer_idx, sb=t.sb)
    return h


def _sync_stacks_back(h: HandState, t: TableState):
    # Persist stack changes from the hand back to table state
    t.stacks = dict(h.stacks)
    # Keep bankroll_base keys aligned
    for nm in list(t.bankroll_base.keys()):
        if nm not in t.stacks:
            t.bankroll_base.pop(nm, None)
    for nm in t.stacks:
        t.bankroll_base.setdefault(nm, t.stacks[nm])


def advance_until_wait(h: HandState, t: TableState):
    """Advance the hand until human input is needed or the hand finishes."""
    try:
        while not h.hand_done:
            betting_round(h, t)
            if t.request_restart:
                h.hand_done = True
                break
            ...
            # Move streets
            if h.street == "preflop":
                reveal_next_street(h)  # flop
                continue
            if h.street == "flop":
                reveal_next_street(h)  # turn
                continue
            if h.street == "turn":
                reveal_next_street(h)  # river
                continue
            if h.street == "river":
                reveal_next_street(h)  # showdown
                continue
            if h.street == "showdown":
                resolve_showdown(h)
                break
        ...
        _sync_stacks_back(h, t)


def card_text(c: int) -> str:
    short = card_label(c)  # e.g., "K♥"
    rs = treys_to_rs(c)
    rank = rs[0]
    suit = rs[1]
...
def render_streamlit(h: HandState, t: TableState):
    st.subheader(f"Hand #{h.hand_no} — Street: {h.street}")
    st.caption(f"Dealer seat: {t.players[h.dealer_idx].name} | SB {h.sb} / BB {h.bb}")

    board = [card_text(c) for c in h.revealed]
    st.markdown("**Board:** " + (" • ".join(board) if board else "(preflop)"))

    rows = []
    for p in t.players:
        nm = p.name
        stack = t.stacks.get(nm, 0)
        ...
        if nm == t.players[h.dealer_idx].name:
            status.append("D")
        if nm == t.players[h.sb_idx].name:
            status.append("SB")
        if nm == t.players[h.bb_idx].name:
            status.append("BB")

        hole = h.hole.get(nm, [])
        show = (nm == t.hero_name) or (h.street == "showdown") or (t.reveal_once and (not p.is_human)) or (t.reveal_folded_once and folded)
        hole_txt = " • ".join(card_text(c) for c in hole) if show and hole else "🂠 🂠"

        rows.append({
            "Player": nm,
            "Type": "" if p.is_human else normalize_type_name(p.ai_type_key or ""),
            "Stack": stack,
            "P/L": f"{pl:+d}",
            "Status": " ".join(status),
            "Cards": hole_txt,
    ...
    st.code("\n".join(h.log[-35:]) if h.log else "(no log yet)")
...
def streamlit_app():
    ...
    with st.sidebar:
        ...
        next_hand = col2.button("Next hand", use_container_width=True)

        st.markdown("---")
        st.caption("Commands: c/call, f/fold, check, r <to>, all in. Cheat: reveal, show out, add <amt>, restart, restart type, explain type, new player <amt>")

    if "t" not in st.session_state or new_game:
        st.session_state.t = _init_table(int(n_ai), int(starting_stack), int(sb))
        st.session_state.dealer_idx = 0
        st.session_state.h = _start_new_hand(st.session_state.t, st.session_state.dealer_idx)

    t: TableState = st.session_state.t
    h: HandState = st.session_state.h

    # Handle restart cheat flags
    if t.request_restart:
        if t.request_restart == "same":
            specs = list(t.initial_specs)
        else:
            specs = list(t.initial_specs)
            ai = [s for s in specs if not s[1]]
            random.shuffle(ai)
            hero = [s for s in specs if s[1]]
            specs = hero + ai

        # rebuild table with same names/stacks
        # use current stacks as starting baseline
        starting_stack = t.starting_stack
        sb = t.sb
        players = []
        stacks = {}
        for name, is_human, ai_key in specs:
            players.append(Player(name=name, is_human=is_human, ai_type_key=ai_key))
            stacks[name] = t.stacks.get(name, starting_stack)
        t2 = TableState(players=players, stacks=stacks, hero_name=t.hero_name, next_ai_id=t.next_ai_id, starting_stack=starting_stack, sb=sb, initial_specs=specs)
        t2.bankroll_base = dict(t.bankroll_base)
        st.session_state.t = t2
        st.session_state.dealer_idx = 0
        st.session_state.h = _start_new_hand(t2, st.session_state.dealer_idx)
        st.session_state.t.request_restart = None
        st.rerun()

    if next_hand and h.hand_done:
        st.session_state.dealer_idx = rotate_dealer(st.session_state.dealer_idx, len(t.players))
        st.session_state.h = _start_new_hand(t, st.session_state.dealer_idx)
        st.rerun()

    # Advance until we need user input
    ...
    with right:
        ...
        if h.hand_done:
            st.write("Hand finished. Use **Next hand** (sidebar) to continue.")
        else:
