...
# --- Dependency bootstrap (fallback) ---
# Streamlit Cloud normally installs from requirements.txt.
# This block is a safety net in case requirements.txt is missing or not picked up.
...
def _ensure_pkg(_name: str, _pip_name: _Optional[str] = None):
    if _ilu.find_spec(_name) is not None:
        return
    _pip = _pip_name or _name
    try:
        _subprocess.check_call([_sys.executable, "-m", "pip", "install", "--no-cache-dir", _pip])
    except Exception as _e:
        raise RuntimeError(
            f"Missing dependency '{_name}'. "
            f"Please add it to requirements.txt (recommended). Tried pip installing '{_pip}' but failed: {_e}"
        )

_ensure_pkg("treys", "treys")
_ensure_pkg("rich", "rich")
# --- End bootstrap ---
...
class TableState:
    ...
    # one-render overlays / reveal
    reveal_once: bool = False
    reveal_folded_once: bool = False  # temporarily show folded players for one render
    overlay_once: bool = False
    overlay_text: Optional[str] = None

    # session P/L baseline
    bankroll_base: Dict[str, int] = field(default_factory=dict)

    # Streamlit-driven human input
